...
productsRouter.put("/:productID", (req, res) => {
  ...
  let updateProductQuery = `UPDATE Products SET title="${body.title}", price=${body.price}, count=${body.count} ,img="${body.img}",popularity=${body.popularity},sale=${body.sale},colors=${body.colors},categoryID=${body.categoryID} WHERE id = ${productID}`;
...
productsRouter.post("/", (req, res) => {
  ...
  let addNewProductQuery = `INSERT INTO Products (title, price, count, img, popularity, sale, colors, productDesc, url, categoryID) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
  let colorsValue = body.colors ? Number(body.colors) : 0;
  let values = [
    body.title, 
    Number(body.price), 
    Number(body.count), 
  ...
  SabzlearnShopDB.query(addNewProductQuery, values, (err, result) => {
    if (err) {
      ...
      return res.status(500).json({ error: "خطا در ثبت محصول", details: err });
    } else {
      res.send(result);
    }
  });
